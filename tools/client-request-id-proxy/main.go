...
func main() {
	...
	fromID := flag.String("from-id", "", "only rewrite requests carrying this client request ID")
	toID := flag.String("to-id", "", "replacement client request ID")
	flag.Parse()

	if strings.TrimSpace(*upstream) == "" || strings.TrimSpace(*fromID) == "" || strings.TrimSpace(*toID) == "" {
		flag.Usage()
