...
func (h *UserHandler) List(c *gin.Context) {
	...
	if bucket := strings.TrimSpace(c.Query("paid_churn")); paidchurn.ValidBucket(bucket) {
		filters.PaidChurnBucket = bucket
	}
...
// GetBalanceHistory handles getting user's balance, concurrency, and subscription history
// GET /api/v1/admin/users/:id/balance-history
// Query params:
//   - type: filter by record type (balance, affiliate_balance, admin_balance, welcome_scratch,
//     surprise_scratch, concurrency, admin_concurrency, subscription)
