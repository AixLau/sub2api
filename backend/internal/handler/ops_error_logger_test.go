...
func TestClassifyOpsAuthClientErrorsExcludedFromSLA(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "localized google invalid API key",
			errType: "api_error",
			message: "API Key 无效",
			code:    "401",
			status:  http.StatusUnauthorized,
		...
		{
			name:    "localized google missing API key",
			errType: "api_error",
			message: "缺少 API Key，请在 Authorization Bearer、x-api-key 或 x-goog-api-key 中提供",
			code:    "401",
			status:  http.StatusUnauthorized,
		},
		{
			name:    "localized google disabled API key",
			errType: "api_error",
			message: "API Key 已停用",
			code:    "401",
			status:  http.StatusUnauthorized,
		...
		{
			name:    "localized google missing API key user",
			errType: "api_error",
		...
		{
			name:    "localized google inactive user",
			errType: "api_error",
			message: "用户账户未启用",
			code:    "401",
			status:  http.StatusUnauthorized,
...
func TestClassifyOpsLocalBusinessLimitErrorsExcludedFromSLA(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "localized google query API key deprecated",
			errType:     "api_error",
			message:     "不再支持通过 URL 查询参数传递 API Key，请改用 Authorization 请求头",
			code:        "400",
			status:      http.StatusBadRequest,
			wantErrType: "api_error",
			wantPhase:   "request",
		...
		{
			name:        "localized google no active subscription",
			errType:     "api_error",
			message:     "当前分组没有可用订阅，请联系管理员开通或续费",
			code:        "403",
			status:      http.StatusForbidden,
			wantErrType: "api_error",
			wantPhase:   "request",
		...
		{
			name:        "localized google daily limit exceeded",
			errType:     "api_error",
			message:     "套餐今日额度已用完，请稍后再试或切换账号",
			code:        "429",
			status:      http.StatusTooManyRequests,
			wantErrType: "api_error",
			wantPhase:   "request",
