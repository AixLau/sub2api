...
func TestSchedulerFullRebuildCapturesAllRegistryTokensBeforeDBLoad(t *testing.T) {
	...
	require.Equal(t, len(schedulerCanonicalBuckets(0))+len(schedulerCanonicalBuckets(61)), captures, "group0 and active-group canonical tokens must be captured before the first DB load")
