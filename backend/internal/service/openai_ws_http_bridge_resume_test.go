...
func TestOpenAIWSHTTPBridgeLaterTurn429RetriesCurrentTurnOnReplacementAccount(t *testing.T) {
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, account, "client-session"), gjson.GetBytes(upstream.bodies[1], "client_metadata.session_id").String())
	...
	require.Equal(t, isolateOpenAIUpstreamSessionID(0, &nextAccount, "client-session"), gjson.GetBytes(upstream.bodies[2], "client_metadata.session_id").String())
