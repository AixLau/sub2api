...
// IsPlatformOperation reports whether a usage row was generated by the
// platform itself rather than an end-user API key.
func (s UsageSource) IsPlatformOperation() bool {
	switch s.Normalize() {
...
func (u *UsageLog) ValidateActors() error {
	...
	if u.Source.Normalize().IsPlatformOperation() {
		if u.UserID != 0 || u.APIKeyID != 0 {
			return fmt.Errorf("%s usage log must not include user_id or api_key_id", u.Source.Normalize())
		}
		return nil
	}
	if u.UserID <= 0 {
