...
func (s *UserSubscription) WeeklyResetTime() *time.Time {
	...
	t := subscriptionWindowBoundary(s.ExpiresAt, s.windowResetAnchor(*s.WeeklyWindowStart), 7*24*time.Hour)
	return &t
}
...
func (s *UserSubscription) MonthlyResetTime() *time.Time {
	...
	t := subscriptionWindowBoundary(s.ExpiresAt, s.windowResetAnchor(*s.MonthlyWindowStart), 30*24*time.Hour)
	return &t
}
