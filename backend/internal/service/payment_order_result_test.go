...
func TestCalculateCreateOrderPayAmountForSubscriptionConvertsCNYPriceWhenRateConfigured(t *testing.T) {
	...
	amountStr, amount, err := calculateCreateOrderPayAmountForPaymentType(payment.TypeAlipay, payment.OrderTypeSubscription, 9.99, 0, 10, "CNY", 7.15)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestCalculateCreateOrderPayAmountForNinePlusSubscriptionUsesProductPaymentAmount(t *testing.T) {
	...
	amountStr, amount, err := calculateCreateOrderPayAmountForPaymentType(payment.TypeNinePlus, payment.OrderTypeSubscription, 50.9, 0, 0.14, "CNY", 7.15)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
