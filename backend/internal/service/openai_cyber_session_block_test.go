...
func (c *comboCacheAndStore) SetUserAccountCooldown(_ context.Context, _ int64, _ int64, _ time.Duration) error {
	return nil
}
...
func TestFindCyberSessionBlocked_CanceledReadFailsOpen(t *testing.T) {
	settingSvc := &SettingService{
	...
	svc := &OpenAIGatewayService{cache: combo, settingService: settingSvc}
	c, body := newCyberBlockTestCtx(map[string]string{"session_id": "session-key"}, `{}`)

	require.Empty(t, svc.FindCyberSessionBlockedForRequest(context.Background(), 1, c, body, "", ""))
}
