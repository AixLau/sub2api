	...
	if cfg.Mode == ContentModerationModePreBlock {
		if cfg.KeywordBlockingMode != ContentModerationKeywordModeAPIOnly {
			if keywordMatch, hit := matchContentModerationKeyword(content.Text, cfg.keywordRules()); hit {
				...
				s.recordPreBlockSyncMetric(0, keywordDecision.action)
				...
				log := s.buildLog(input, cfg, keywordDecision.action, keywordDecision.flagged, contentModerationKeywordCategory, 1.0, scores, content.ExcerptText(), nil, nil, "")
				...
				decision := contentModerationDecisionFromKeyword(cfg, keywordDecision, scores)
				if keywordDecision.blocked || cfg.KeywordBlockingMode == ContentModerationKeywordModeKeywordOnly {
					return decision, nil
				}
			}
		}
...
func decideContentModerationKeyword(text string, rule ContentModerationKeywordRule) contentModerationKeywordDecision {
	...
	if shouldDowngradeKeywordActionForContext(ctx) {
	...
	if effectiveAction == ContentModerationKeywordActionBlock {
