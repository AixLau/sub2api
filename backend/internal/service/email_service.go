...
func (s *EmailService) sendEmailWithConfig(ctx context.Context, config *SMTPConfig, to, subject, body string) error {
	...
	client, stopContextClose, err := s.connectSMTP(ctx, config)
	if err != nil {
		return err
	}
	defer func() {
...
func (s *EmailService) connectSMTP(ctx context.Context, config *SMTPConfig) (*smtp.Client, func(), error) {
...
func (s *EmailService) connectSMTPStartTLS(ctx context.Context, dialer *net.Dialer, addr, host string, tlsConfig *tls.Config, mandatory bool) (*smtp.Client, func(), error) {
	conn, err := dialer.DialContext(ctx, "tcp", addr)
	if err != nil {
		return nil, nil, fmt.Errorf("smtp dial: %w", err)
	}
	client, stopContextClose, err := newSMTPClient(ctx, conn, host)
	if err != nil {
		return nil, nil, err
	}
...
func newSMTPClient(ctx context.Context, conn net.Conn, host string) (*smtp.Client, func(), error) {
	stopContextClose := prepareSMTPConnection(ctx, conn)
	client, err := smtp.NewClient(conn, host)
	if err != nil {
		stopContextClose()
		_ = conn.Close()
		return nil, nil, fmt.Errorf("new smtp client: %w", err)
	}
	return client, stopContextClose, nil
}
...
func (s *EmailService) TestSMTPConnectionWithConfig(config *SMTPConfig) error {
	client, stopContextClose, err := s.connectSMTP(context.Background(), config)
	if err != nil {
