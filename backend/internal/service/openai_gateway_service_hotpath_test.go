...
func TestOpenAIGatewayService_Forward_APIKeyCustomBaseURLStripsMaxOutputTokensWithForceCodexCLI(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
	...
	account := &Account{
		ID:          12,
		Name:        "openai-apikey-custom",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Concurrency: 1,
		Credentials: map[string]any{
			"api_key":  "sk-test",
			"base_url": "https://tuokingmi.cn",
		},
		Extra: map[string]any{"use_responses_api": true},
	}
	rec := httptest.NewRecorder()
	...
	body := []byte(`{"model":"gpt-5.5","stream":false,"max_output_tokens":512,"input":[{"type":"message","content":"hi"}]}`)
	result, err := svc.Forward(context.Background(), c, account, body)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, "https://tuokingmi.cn/v1/responses", upstream.lastReq.URL.String())
	require.False(t, gjson.GetBytes(upstream.lastBody, "max_output_tokens").Exists())
}

func TestOpenAIGatewayService_Forward_APIKeyOfficialBaseURLKeepsMaxOutputTokens(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
	...
	require.Equal(t, int64(512), gjson.GetBytes(upstream.lastBody, "max_output_tokens").Int())
}
