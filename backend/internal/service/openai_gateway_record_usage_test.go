...
func TestOpenAIGatewayServiceRecordUsage_PrefersUpstreamRequestIDOverReusedClientRequestID(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
...
func TestOpenAIGatewayServiceRecordUsage_FallsBackToResponseIDWhenUpstreamRequestIDMissing(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	ctx := context.WithValue(context.Background(), ctxkey.ClientRequestID, "openai-client-reused-123")
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			ResponseID: "resp_openai_http_turn_456",
			Usage: OpenAIUsage{
				InputTokens:  8,
				OutputTokens: 4,
			},
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "resp_openai_http_turn_456", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "resp_openai_http_turn_456", usageRepo.lastLog.RequestID)
}
...
func TestOpenAIGatewayServiceRecordUsage_ServiceTierPriorityUsesFastPricing(t *testing.T) {
	...
	require.InDelta(t, baseCost.TotalCost*2.5, usageRepo.lastLog.TotalCost, 1e-10)
}
...
func TestOpenAIGatewayServiceRecordUsage_ChannelMappedUsesGenericFinalMappingTargetPricing(t *testing.T) {
	const (
		groupID        = int64(1118)
	...
	channelOutputPrice := 30e-6

	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, userRepo, subRepo, nil)
	// Reproduce the regression: the public alias has a resolvable global price,
	// while the mapped target has the administrator's channel price.
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID:     "resp_generic_mapped_target_pricing",
