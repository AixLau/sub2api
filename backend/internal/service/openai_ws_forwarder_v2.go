...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	applyStagedCodexFingerprintHeaders(c, account, identityHeaders)
	identityFallback := strings.TrimSpace(clientPromptCacheKey)
	if identityFallback == "" {
		identityFallback = openAIWSPayloadString(payload, "prompt_cache_key")
	}
	codexIdentity, _, identityErr := normalizeCodexOutboundIdentityMap(identityHeaders, payload, identityFallback)
	if identityErr != nil {
	...
	applyCodexOutboundIdentityToHeaders(wsHeaders, codexIdentity)
	logOpenAIWSModeDebug(
