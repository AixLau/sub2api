...
// ---- DeepSeek 官方低谷价（$/token，2026-08-16 16:00 UTC 起生效）----
...
// isDeepSeekModel 仅识别已有明确价卡的 DeepSeek V4 家族及历史兼容别名。
// 未知 deepseek-* 保持 fail-closed，避免在官方价卡尚未确认时误扣费用。
func isDeepSeekModel(model string) bool {
	return deepseekPricingFamily(model) != ""
}

func deepseekPricingFamily(model string) string {
	...
	case strings.Contains(normalized, "deepseek-v4-flash"):
		return "flash"
	case normalized == "deepseek-chat", normalized == "deepseek-reasoner":
		return "flash"
	default:
		return ""
	}
...
// 官方口径（2026-08-16 16:00 UTC 起生效）：高峰价 = 2× 低谷价；高峰时段为
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	// 历史 deepseek-chat / deepseek-reasoner 按 flash 价兼容；未知型号保持
	// fail-closed，等待显式价卡后再开放计费。
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	// （2026-08-16 16:00 UTC 起生效）。这是覆盖远端旧价的关键——远端仓库不可改，生产会先
	// 拉到旧价，必须在此兜底修正；克隆后再覆盖，避免污染共享 fallbackPrices 指针。
	// 档位判定：V4 Pro 的版本化名称归 pro，V4 Flash 及历史兼容别名归 flash；
	// 未知 deepseek-* 不进入该路径，继续由默认计费查找 fail-closed。
	// 高峰时段倍率不在本函数处理，由 calculateTokenCost 按 deepseekPeakMultiplierAt
	// 对默认价卡另行叠加（分组/渠道自定义定价不叠加）。
	if family := deepseekPricingFamily(model); forceDeepSeekRates && family != "" {
		cloned := *pricing
		if family == "pro" {
			...
			// deepseek-v4-flash / vision-exp 与历史兼容别名共用 flash 价。
