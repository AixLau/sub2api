...
func (s *SubscriptionService) AssignOrMergeSubscriptionPurchase(ctx context.Context, input *AssignSubscriptionInput) (*AssignSubscriptionResult, error) {
	return s.assignOrMergeSubscriptionPurchase(ctx, input, false)
}

func (s *SubscriptionService) assignOrMergeSubscriptionPurchase(ctx context.Context, input *AssignSubscriptionInput, deferCacheInvalidation bool) (*AssignSubscriptionResult, error) {
	if input == nil {
...
func (s *SubscriptionService) ValidateAndCheckLimits(sub *UserSubscription, group *Group) (needsMaintenance bool, err error) {
	now := s.now()
	if pendingRenewalDue(sub, group, now) {
		return true, nil
	}
	// 1. 验证订阅状态
	if sub.Status == SubscriptionStatusExpired {
