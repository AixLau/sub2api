...
var supplementalPatternConfigs = []PatternConfig{
	...
	},
	{
		...
		Regex:    `(?i)\b(?:ida(?:\s+pro)?|ghidra|x64dbg|ollydbg|frida|jadx|apktool|decompil(?:e|er)|disassembl(?:e|y)|reverse\s+engineer(?:ing)?)\b.{0,120}\b(?:extract|dump|recover|decrypt|steal)\b.{0,80}\b(?:api\s*keys?|tokens?|secrets?|private\s*keys?|passwords?|certificates?|license\s*keys?)\b|\b(?:ida(?:\s+pro)?|ghidra|x64dbg|ollydbg|frida|jadx|apktool|decompil(?:e|er)|disassembl(?:e|y)|reverse\s+engineer(?:ing)?)\b.{0,120}\b(?:bypass|crack|patch|remove|unlock)\b.{0,80}\b(?:license|activation|trial|paywall|subscription|in[-\s]?app\s+purchase|iap|entitlement|signature\s+check)\b|(?:IDA|Ghidra|x64dbg|Frida|JADX|APKTool|反编译|反汇编|逆向).{0,100}(?:提取|导出|解密|恢复|窃取).{0,50}(?:API密钥|密钥|令牌|密码|私钥|证书|授权码)|(?:IDA|Ghidra|x64dbg|Frida|JADX|APKTool|反编译|反汇编|逆向).{0,100}(?:绕过|破解|补丁|去除|解锁).{0,50}(?:授权|激活|试用|付费墙|会员|订阅|内购|权益|签名校验)`,
		Weight:   100,
		Category: "reverse_engineering",
		Strict:   true,
	},
