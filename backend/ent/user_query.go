...
// QueryRewardGrants chains the current query on the "reward_grants" edge.
func (_q *UserQuery) QueryRewardGrants() *UserRewardGrantQuery {
...
// QueryRewardCampaignStates chains the current query on the "reward_campaign_states" edge.
func (_q *UserQuery) QueryRewardCampaignStates() *RewardCampaignUserStateQuery {
...
// QueryBehaviorDaily chains the current query on the "behavior_daily" edge.
func (_q *UserQuery) QueryBehaviorDaily() *UserBehaviorDailyQuery {
	query := (&UserBehaviorDailyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithRewardGrants tells the query-builder to eager-load the nodes that are connected to
// the "reward_grants" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRewardGrants(opts ...func(*UserRewardGrantQuery)) *UserQuery {
	query := (&UserRewardGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRewardGrants = query
	return _q
}

// WithRewardCampaignStates tells the query-builder to eager-load the nodes that are connected to
// the "reward_campaign_states" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRewardCampaignStates(opts ...func(*RewardCampaignUserStateQuery)) *UserQuery {
	query := (&RewardCampaignUserStateClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRewardCampaignStates = query
	return _q
}

// WithBehaviorDaily tells the query-builder to eager-load the nodes that are connected to
// the "behavior_daily" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBehaviorDaily(opts ...func(*UserBehaviorDailyQuery)) *UserQuery {
	query := (&UserBehaviorDailyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBehaviorDaily = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRewardGrants; query != nil {
		if err := _q.loadRewardGrants(ctx, query, nodes,
			func(n *User) { n.Edges.RewardGrants = []*UserRewardGrant{} },
			func(n *User, e *UserRewardGrant) { n.Edges.RewardGrants = append(n.Edges.RewardGrants, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRewardCampaignStates; query != nil {
		if err := _q.loadRewardCampaignStates(ctx, query, nodes,
			func(n *User) { n.Edges.RewardCampaignStates = []*RewardCampaignUserState{} },
			func(n *User, e *RewardCampaignUserState) {
				n.Edges.RewardCampaignStates = append(n.Edges.RewardCampaignStates, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := _q.withBehaviorDaily; query != nil {
...
func (_q *UserQuery) loadRewardGrants(ctx context.Context, query *UserRewardGrantQuery, nodes []*User, init func(*User), assign func(*User, *UserRewardGrant)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userrewardgrant.FieldUserID)
	}
	query.Where(predicate.UserRewardGrant(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RewardGrantsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadRewardCampaignStates(ctx context.Context, query *RewardCampaignUserStateQuery, nodes []*User, init func(*User), assign func(*User, *RewardCampaignUserState)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(rewardcampaignuserstate.FieldUserID)
	}
	query.Where(predicate.RewardCampaignUserState(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RewardCampaignStatesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadBehaviorDaily(ctx context.Context, query *UserBehaviorDailyQuery, nodes []*User, init func(*User), assign func(*User, *UserBehaviorDaily)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userbehaviordaily.FieldUserID)
	}
