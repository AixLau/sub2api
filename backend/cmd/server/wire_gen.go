...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	accountRepository, err := repository.ProvideAccountRepository(client, db, schedulerCache, configConfig)
	if err != nil {
		return nil, err
	}
	...
	subscriptionService := service.ProvideSubscriptionService(groupRepository, userSubscriptionRepository, billingCacheService, client, configConfig, apiKeyAuthCacheInvalidator)
	...
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, apiKeyRepository, affiliateService)
	...
	rewardService := service.NewRewardService(rewardRepository, apiKeyAuthCacheInvalidator, billingCache)
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, rewardService)
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyService)
	clientSetupHandler := handler.ProvideClientSetupHandler(redisClient, apiKeyService)
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(accountRepository, configConfig, proxyRepository, openAIOAuthClient, privacyClientFactory)
	...
	openAIGatewayService := service.NewOpenAIGatewayService(proxyExitInfoProber, accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository)
	...
	adminAccountRepository, err := repository.ProvideAdminAccountRepository(client, db, schedulerCache, configConfig)
	if err != nil {
		return nil, err
	}
	proxyLatencyCache := repository.NewProxyLatencyCache(redisClient)
	adminService := service.NewAdminService(configConfig, userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, usageLogRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver, channelService)
	...
	accountHandler := admin.ProvideAccountHandler(configConfig, adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, credentialImportService, credentialPrincipalCreator)
	adminAnnouncementHandler := admin.NewAnnouncementHandler(announcementService)
	adminRewardHandler := admin.NewRewardHandler(rewardService)
	...
	tokenRefreshService := service.ProvideTokenRefreshService(accountRepository, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, compositeTokenCacheInvalidator, schedulerCache, configConfig, tempUnschedCache, proxyRepository, oAuthRefreshAPI, openAIGatewayService)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, apiKeyRepository, apiKeyService, affiliateService, notificationEmailService)
	...
	handlerMerchantSSOHandler := handler.NewMerchantSSOHandler(merchantSSOService)
	merchantSSOAPIService := service.NewMerchantSSOAPIService(client, userRepository, authService, configConfig)
	merchantSSOAPIHandler := handler.NewMerchantSSOAPIHandler(merchantSSOAPIService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	rewardJobWorker := service.ProvideRewardJobWorker(rewardRepository, settingService)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, clientSetupHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, rewardHandler, channelMonitorUserHandler, channelMonitorV2Handler, publicTransitHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, handlerMerchantSSOHandler, merchantSSOAPIHandler, idempotencyCoordinator, idempotencyCleanupService, rewardJobWorker)
