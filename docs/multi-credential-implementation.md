...
负载冒烟：本地 OrbStack 4GB、PostgreSQL实际18.4、三 worker，每组合5秒。C10/I3 p95=37.89ms，C10/I16=32.32ms，C50/I3=28.04ms，C50/I16=30.64ms，C200/I3=30.28ms，C200/I16=30.83ms；无测试发现的漏释放。**此段仅记录当时的冒烟：当时未达到20ms、未运行10分钟矩阵。后续E1已补齐六组合，当前状态以专项E2及验收账本为准；历史段不代表当前运行状态。** 日志 `/tmp/sub2api-credential-load-smoke.log`。数据库在Docker VM、SQL往返多且主体串行锁，需进一步按事务阶段剖析，不据此改变权威账本选型。

## 验收收尾更正

起点见 `credential-acceptance/baseline.md`。先前“七阶段均完成”及“全套失败均属既有问题”的概括撤回：仅部分失败已对照，必须逐项比较。验收结论以逐项 AT 账本与证据为准。

## 准入时间与性能专项（以5effa833为起点）

没有扩展调度功能。实际发现并修复锁等待后的deadline/heartbeat事务时间问题与HTTP detached context的取消边界；补充31秒真实锁等待、pool取消、未发送预留补偿、dispatch提交未知保留占用。依据分阶段测量，仅增加活跃ledger索引、无竞争票据短路、已持锁行的批量UPDATE，保留权威核对和锁顺序。功能仍默认关闭。各修改独立提交、命令/失败/迁移/回滚详见 [专项报告](credential-acceptance/admission-time-performance.md)，受测版本和报告版本分别记录。性能未达20ms时继续保留发布阻断，不调高门槛。
