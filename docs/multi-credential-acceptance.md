...
当前最终 HEAD：见 [baseline](credential-acceptance/baseline.md) 更新后的提交历史；验收收尾提交包括 `5f335e11b`、`515b23bc9`、`393655edd`、`039ef1cf2`、`85c47db9f`、`b80c9b9a0`、`53164d062`、`8434b6748`。
...
- `go test ./...`：当前最终 HEAD 实际退出 1。基线 `fde7e8ec4` 与当前最终 HEAD 各自完整运行均记录 78 个失败事件、141 个父测试中断/未完成事件；完整逐测试对照见 [`full-suite-comparison-final.json`](credential-acceptance/full-suite-comparison-final.json)，逐项复跑证据见 [`failure-comparison.md`](credential-acceptance/failure-comparison.md)。相同结果不能自动归因基线；WS/流错误代表性失败已单测复跑，仍有其他范围外失败需要单独归因。
