...
- The two allow-direction downgrades (`semantic_policy_harmless_review` and `semantic_policy_unsubstantiated_fraud`) run only when evidence is complete. Under incomplete (e.g. truncated) candidate evidence, a model `review` is kept as `review` for human triage, and the model's original severity is preserved in the `semantic_review_model_severity` metadata field whenever a policy forces the verdict to `allow`.
