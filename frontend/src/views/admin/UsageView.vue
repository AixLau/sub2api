  ...
  try {
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.upstream_model || log.model, log.upstream_response_model || '', log.upstream_model_mismatch == null ? '' : t(log.upstream_model_mismatch ? 'common.yes' : 'common.no'), formatReasoningEffort(log.requested_reasoning_effort || log.reasoning_effort), formatReasoningEffort(log.reasoning_effort), log.group?.name || '',
...
const allColumns = computed(() => [
  ...
  { key: 'reasoning_effort', label: t('usage.reasoningEffort') === 'usage.reasoningEffort' ? '推理强度' : t('usage.reasoningEffort'), sortable: false },
