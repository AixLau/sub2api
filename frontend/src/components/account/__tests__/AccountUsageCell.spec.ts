...
describe('AccountUsageCell', () => {
  ...
  it('OpenAI OAuth 主动查询只更新当前账号用量窗口', async () => {
    ...
    await flushPromises()
    getUsage.mockClear()
    const activeQueryButton = wrapper.findAll('button').find(
      (button) => button.text().includes('admin.accounts.usageWindow.activeQuery')
    )
    expect(activeQueryButton).toBeDefined()

    await activeQueryButton!.trigger('click')
    await flushPromises()
