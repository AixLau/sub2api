<template>
  <div class="relative" ref="dropdownRef">
    <button
      ...
      :disabled="switching"
      class="flex h-9 w-9 items-center justify-center rounded-full text-gray-600 transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:text-gray-300 dark:hover:bg-dark-700 dark:focus:ring-offset-dark-900"
      :title="currentLocale?.name"
      :aria-label="currentLocale?.name"
      :aria-expanded="isOpen"
      aria-haspopup="menu"
    >
      <span class="text-lg leading-none" aria-hidden="true">{{ currentLocale?.flag }}</span>
    </button>
