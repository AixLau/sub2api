export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'When multiple users share an OAuth account, Device only uses one stable device. For ordinary HTTP Responses and HTTP passthrough, Device + Session combines normal tasks into one session per user per fixed 5–7 day period. Each /side keeps its own stable session and cache root; child agents inherit their root session and cache. Thread references and the client turn graph are preserved. New OpenAI OAuth accounts default to Device only; other modes require an explicit choice. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
