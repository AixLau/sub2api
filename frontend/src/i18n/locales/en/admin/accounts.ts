export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'When multiple users share an OAuth account, Device only uses one stable device. For ordinary HTTP Responses and HTTP passthrough, Device + Session gives each downstream user one session per fixed 5–7 day period, each task its own thread, and preserves the client turn graph. Child agents share their root cache; side threads use a separate cache root. New OpenAI OAuth accounts default to Device only; other modes require an explicit choice. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
