export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'When multiple users share an OAuth account, Device only uses one stable device. For ordinary HTTP Responses and HTTP passthrough, Device + Session combines normal tasks into one session per user per fixed 5–7 day period; normal threads change with that session. Each /side keeps its own stable session, threads and cache root, with its fork target fixed at creation. Child agents inherit their root session and cache. After a normal period changes, resume the parent before its children; forks require a known source thread. The client turn graph is preserved. New OpenAI OAuth accounts default to Device only; other modes require an explicit choice. Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
