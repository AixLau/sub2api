    ...
    ops: {
      ...
      errorDetail: {
        ...
        diagnosis: {
          ...
          streamTimeout: '流数据等待超时',
          streamTimeoutSummary: '流数据等待超过空闲阈值。上游停顿、网络中断和插件缓冲均可能造成此现象，当前记录不足以确定责任方。',
          streamTimeoutEvidence: '结构化错误码：stream_timeout；展示的 502 不能证明上游曾返回 HTTP 502。',
          streamTimeoutAction: '对照上游数据到达、插件转发和网关超时的时间线；检查缓冲期间是否传递真实数据活动，再判断是否需要调整超时配置。',
      ...
      alertRules: {
        ...
        metrics: {
          ...
          memory: '内存使用率 (%)',
          queueDepth: '并发排队深度',
          contentModerationPendingReviewAge: '最老内容审核待复核时长（秒）',
        ...
        metricDescriptions: {
          ...
          contentModerationPendingReviewAge: '等待人工复核时间最久的内容审计记录年龄（秒）。',
