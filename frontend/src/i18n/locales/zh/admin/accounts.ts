export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，「仅设备」统一为一个稳定设备。普通 HTTP Responses 和 HTTP 透传的「设备+会话」模式下，每个用户的普通任务在固定 5～7 天周期内共用一个会话；每个 /side 保留独立稳定会话与 cache root，子 Agent 继承所属 root 的会话和 cache。线程引用与客户端 turn 关系保持完整。新建 OpenAI OAuth 账号默认选择「仅设备」，其他模式需手动选择。部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
