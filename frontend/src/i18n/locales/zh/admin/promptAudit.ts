export default {
  promptAudit: {
    ...
    runtime: {
      ...
      deliveryTotals: '累计入队 {enqueued} · 重复跳过 {deduplicated} · 丢弃 {dropped} · 处理 {processed} · 失败 {failed}',
    },
    ...
    policy: {
      selectedAccountCount: '已选择 {count} 个账号',
      accountScope: '上游账号范围',
      accountScopeHint: '与分组范围取交集，按实际选中的上游账号审计。指定账号只可选择 OpenAI OAuth 类型。',
      allAccounts: '全部账号',
      selectedAccounts: '指定账号',
      searchAccounts: '搜索账号名称或 ID',
      noAccounts: '没有可选的 OpenAI OAuth 账号',
      missingAccounts: '以下账号不存在或不再是 OpenAI OAuth 类型，请重新选择',
      clearMissingAccounts: '移除无效选择',
      loadAccountsError: '加载账号列表失败',
    ...
    events: {
      title: '审计事件', description: '按身份、入口、风险、Hash 和时间复核事件，详情中可查看完整提示词。', decision: '判定', risk: '风险等级', endpoint: '入口', groupId: '分组 ID', userEmail: '用户邮箱', apiKeyId: 'API Key ID', keyword: '关键词',
