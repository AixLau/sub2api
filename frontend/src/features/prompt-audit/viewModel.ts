...
export function buildUpdateRequest(draft: PromptAuditDraft): PromptAuditUpdateRequest {
  return {
    ...
    medium_risk_allows_next_stage: draft.medium_risk_allows_next_stage,
    high_risk_allows_next_stage: draft.high_risk_allows_next_stage,
    max_attempts: Math.max(1, Math.min(10, Number(draft.max_attempts) || 3)),
...
export function eventQueryParams(filters: PromptEventFilters): Record<string, string | number> {
  ...
  for (const key of ['decision', 'risk_level', 'endpoint', 'request_id', 'prompt_hash', 'keyword'] as const) {
	...
	const email = filters.user_email.trim()
	if (email) result.user_email = email
